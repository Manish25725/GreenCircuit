...
const dataVolumeTracker = new Map<string, { bytes: number; resetTime: number }>();
...
export const antiDataFlood = (req: Request, res: Response, next: NextFunction) => {
...
export const validateDataStructure = (req: Request, res: Response, next: NextFunction) => {
  const maxDepth = 10;
  ...
  const checkDepth = (obj: any, depth: number = 0): boolean => {
    ...
    if (Array.isArray(obj)) {
      ...
      return obj.every(item => 
        typeof item !== 'object' || item === null || checkDepth(item, depth + 1)
      );
    }
    
    if (typeof obj === 'object' && obj !== null) {
      const keys = Object.keys(obj);
      if (keys.length > maxObjectKeys) return false;
      return keys.every(key => 
        typeof obj[key] !== 'object' || obj[key] === null || checkDepth(obj[key], depth + 1)
      );
    }
...
export const validateStringLengths = (req: Request, res: Response, next: NextFunction) => {
  ...
  const checkStrings = (obj: any, path: string = ''): string | null => {
    for (const key in obj) {
