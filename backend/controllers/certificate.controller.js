    ...
    try {
        ...
        sendSuccess(res, {
            ...
            certificate: {
                ...
                agencyName: certificate.agencyId.name
            }
        });
    }
    catch (error) {
...
export const downloadCertificate = async (req, res) => {
    try {
        ...
        const userName = certificate.userId?.name || 'User';
        const userEmail = certificate.userId?.email || '';
        const agencyName = certificate.agencyId?.name || 'Authorized Agency';
        ...
        // Waste Categories
        const wasteTypes = certificate.itemsRecycled?.map((item) => item.type).join(', ') || 'E-Waste';
