...
export const updateBusinessProfile = async (req, res) => {
    try {
        ...
        const business = await Business.findOneAndUpdate({ userId }, updates, { new: true, runValidators: true });
        if (!business) {
...
export const updateInventoryItem = async (req, res) => {
    try {
        ...
        const item = await Inventory.findOneAndUpdate({ _id: id, businessId: business._id }, updateData, { new: true, runValidators: true });
        if (!item) {
            return sendError(res, 'Item not found', 404);
        }
        sendSuccess(res, item);
...
export const bulkUpdateInventoryStatus = async (req, res) => {
    try {
        const userId = req.user._id || req.user.id;
        ...
        const result = await Inventory.updateMany({ _id: { $in: itemIds }, businessId: business._id }, { status });
        sendSuccess(res, {
...
export const markItemsForPickup = async (req, res) => {
    try {
        const userId = req.user._id || req.user.id;
        ...
        await Inventory.updateMany({ _id: { $in: itemIds }, businessId: business._id }, { status: 'ready-for-pickup' });
...
export const downloadCertificate = async (req, res) => {
    try {
        ...
        const agencyName = certificate.agencyId?.name || certificate.issuedBy?.name || 'Authorized Agency';
        ...
        if (certificate.complianceStandards && certificate.complianceStandards.length > 0) {
            ...
            let tagX = 50;
            tableY += 18;
            certificate.complianceStandards.forEach((standard) => {
...
export const scheduleBusinessPickup = async (req, res) => {
    try {
        const userId = req.user._id || req.user.id;
        ...
        if (booking && inventoryItemIds && inventoryItemIds.length > 0) {
            await Inventory.updateMany({ _id: { $in: inventoryItemIds }, businessId: business._id }, { status: 'scheduled', bookingId: booking._id });
        }
